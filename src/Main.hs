...
bCode Code{..} = do
    ...
    let lbs = toLazyByteString . mconcat . map bBytecode $ bcs
...
toBytecode i = case i of
    ...
    IConst x | x >= minI8  && x <= maxI8  -> pure (B'BIPush (fromIntegral x))
             ...
             | otherwise                  -> error "toBytecode: todo: int32 constants"
...
bBytecode bc = case bc of
    ...
    B'TableSwitch         -> error "bBytecode: not implemented 0xaa"
    B'LookupSwitch        -> error "bBytecode: not implemented 0xab"
    ...
    B'Wide                -> error "bBytecode: not implemented 0xc4"
